...
def visualize_gpm(supplier_gpm):
    """
    Visualizes the Gross Profit Margin (GPM) across supplier categories.

    Args:
      supplier_gpm: A dictionary containing GPM for each supplier category.
    """
    
    colors = plt.cm.viridis(range(len(supplier_gpm)))  # Generate color map
    plt.figure(figsize=(12, 6))
    plt.bar(supplier_gpm.keys(), supplier_gpm.values(), color=colors)

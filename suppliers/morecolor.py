...
def visualize_order_counts(data):
    """
    Visualizes the number of orders per supplier category with unique colors.
    ...
    categories = order_counts.index.tolist()
    colors = plt.cm.viridis(np.linspace(0, 1, len(categories)))  # Use colormap

    plt.figure(figsize=(12, 6))
    plt.bar(order_counts.index, order_counts.values, color=colors)
    plt.xlabel('Supplier Category')

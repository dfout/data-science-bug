...
def analyze_log(log_file):
    ...
    # Get the union of all unique nodes that have failures or not responding events
    all_nodes = sorted(set(node_failures.keys()) | set(not_responding_nodes.keys()))

    # Create a dataframe for analysis and plotting
    df = pd.DataFrame({
        'Node': all_nodes,
        'Failures': [node_failures.get(node, 0) for node in all_nodes],
        'Not Responding': [not_responding_nodes.get(node, 0) for node in all_nodes]
    })
